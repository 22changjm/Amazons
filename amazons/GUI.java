...
class GUI extends TopLevel implements View, Reporter {
    ...
    GUI(String title) {
        ...
        addMenuButton("Settings->ChangePlayerAuto", this::changeAutoPlayer);
        addMenuButton("Settings->ChangePlayerManual", this::changeManualPlayer);
    ...
    /** Change player to auto. */
    private void changeAutoPlayer(String dummy) {
        String response =
                getTextInput("White or Black?",
                        "Change Auto", "plain", "");
        if (response != null) {
            response = response.toLowerCase();
            if (response.equals("white") || response.equals("black")) {
                _pendingCommands.offer("auto " + response);
            } else {
                showMessage("Enter White or Black", "Error", "error");
            }
        }
    }
    /** Change player to manual. */
    private void changeManualPlayer(String dummy) {
        String response =
                getTextInput("White or Black?",
                        "Change Manual", "plain", "");
        if (response != null) {
            response = response.toLowerCase();
            if (response.equals("white") || response.equals("black")) {
                _pendingCommands.offer("manual " + response);
            } else {
                showMessage("Enter White or Black", "Error", "error");
            }
        }
