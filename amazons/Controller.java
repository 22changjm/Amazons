...
final class Controller {
     ...
     *  with the value SEED. Identical seeds produce idencal sequences.
    ...
    private Command[] _commands = {
        ...
        new Command("([a-z]\\d+ [a-z]\\d+ [a-z]\\d+$)|"
                + "([a-z]\\d+\\-[a-z]\\d+\\([a-z]\\d+\\)$)", this::doMove),
        new Command("(auto\\s+(?)white$)"
                + "|(auto\\s+(?)black$)", this::doAuto),
        new Command("(manual\\s+(?)white$)"
                + "|(manual\\s+(?)black$)", this::doManual),
        new Command("undo$", this::doRedo),
        new Command("nummoves$", this::donumMoves)
    };
    ...
    /** Performs undo on the board. */
    private void doRedo(Matcher unused) {
        if (((_white instanceof AI) && !(_black instanceof AI))
                || ((_black instanceof AI) && !(_white instanceof AI))) {
            _board.undo();
        }
        _board.undo();
    }
    /** Displays number of moves on board. */
    private void donumMoves(Matcher unused) {
        System.out.println(board().numMoves());
    }

    /** Changes player to manual.
     * @param mat input */
    private void doManual(Matcher mat) {
        String s = mat.group(0);
        String side = s.substring(6);
    ...
    /** Changes player to auto.
     * @param mat input */
    private void doAuto(Matcher mat) {
        String s = mat.group(0);
        String side = s.substring(4);
        side = side.replaceAll(" ", "");
        side = side.toLowerCase();
        if (side.equals("white")) {
    ...
    private void doMove(Matcher mat) {
        ...
        String from = list[0];
        String to = list[1];
        String spear = list[2].replaceAll("\\)", "");
        board().makeMove(sq(from), sq(to), sq(spear));
